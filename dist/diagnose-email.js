...
async function diagnoseEmail() {
    ...
    console.log('   SMTP_HOST:', env_1.env.SMTP_HOST || '❌ NOT SET');
    console.log('   SMTP_PORT:', env_1.env.SMTP_PORT || '❌ NOT SET');
    console.log('   SMTP_USER:', env_1.env.SMTP_USER || '❌ NOT SET');
    console.log('   SMTP_PASSWORD:', env_1.env.SMTP_PASSWORD ? '✅ SET (hidden)' : '❌ NOT SET');
    console.log('   EMAIL_FROM:', env_1.env.EMAIL_FROM || '❌ NOT SET');
    console.log('   EMAIL_FROM_NAME:', env_1.env.EMAIL_FROM_NAME || '❌ NOT SET');
    ...
    try {
        ...
        console.log('   Connecting to', env_1.env.SMTP_HOST, 'on port', env_1.env.SMTP_PORT, '...');
        await transporter.verify();
        console.log('   ✅ SMTP connection successful!');
        // Step 3: Send test email
        console.log('\n3️⃣ SENDING TEST EMAIL:');
        console.log('   From:', env_1.env.EMAIL_FROM);
        console.log('   To:', env_1.env.SMTP_USER);
        const info = await transporter.sendMail({
            from: `"${env_1.env.EMAIL_FROM_NAME}" <${env_1.env.EMAIL_FROM}>`,
            to: env_1.env.SMTP_USER, // Send to yourself
      ...
      `,
        ...
        console.log('\n📬 CHECK YOUR INBOX:', env_1.env.SMTP_USER);
